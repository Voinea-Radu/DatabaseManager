    ...
    public void createTable(Class<? extends DatabaseEntry<?>> clazz) {
        LambdaExecutor.LambdaCatch.NoReturnLambdaCatch.executeCatch(() -> {
            ...
            for (Field field : fields) {
                ...
                boolean autogenerate = dbField.autoGenerate();

                if (!autogenerate && field.getName().equals("id") && clazz.newInstance().isIDAutoGenerated()) {
                        autogenerate = true;
                }

                placeholder += dbField.columnName() + " " + getDataType(field.getType()) + " " + (dbField.unique() ? "UNIQUE " : "") + (autogenerate ? sqlConfig.driver(
                        main).autoIncrement : "") + ",";
        ...
        LambdaExecutor.LambdaCatch.NoReturnLambdaCatch.executeCatch(() -> {
            ...
            if (entry.getID() != null) {
                ...
                for (Field field : fields) {
                    ...
                    if (field.getName().equals("id") && entry.isIDAutoGenerated()) {
                        continue;
                    }
